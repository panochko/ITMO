...
public class ServerManager {
                ...
                readingPool.submit(() -> {
                    try (var byteArrayInputStream = new ByteArrayInputStream(receivedBytes);
                        ...
                        processingPool.submit(() -> {
                            try {
                                processCommand(container, commandName, inetAddress, clientPort, datagramSocket);
                            } catch (Exception e) {
    ...
    private static void processCommand(Container<? extends AbstractCommand> container, String commandName,
                                       InetAddress inetAddress, int clientPort, DatagramSocket datagramSocket) throws Exception {
        ...
        if (commandName.matches("filter_contains_name [A-Za-zА-Яа-я ,-.]+") ||
            ...
            handleRemoveById(container, inetAddress, clientPort, datagramSocket);
        }
        else if (commandName.matches("update [0-9]+")) {
            handleUpdate(container, inetAddress, clientPort, datagramSocket);
        }
        else if (commandName.matches("remove_greater [0-9]+")) {
            handleRemoveGreater(container, inetAddress, clientPort, datagramSocket);
        }
            ...
            switch (commandName) {
                case "add" -> handleAdd(container, inetAddress, clientPort, datagramSocket);
                case "add_if_max" -> handleAddIfMax(container, inetAddress, clientPort, datagramSocket);
                case "add_if_min" -> handleAddIfMin(container, inetAddress, clientPort, datagramSocket);
                case "clear" -> handleClear(container, inetAddress, clientPort, datagramSocket);
                case "help" -> handleHelp(container, inetAddress, clientPort, datagramSocket);
                case "min_by_coordinates" -> handleMinByCoordinates(container, inetAddress, clientPort, datagramSocket);
                case "info" -> handleInfo(container, inetAddress, clientPort, datagramSocket);
                case "show" -> handleShow(container, inetAddress, clientPort, datagramSocket);
                default -> sendResponse(Color.RED + "Неизвестная команда: " + commandName, inetAddress, clientPort, datagramSocket);
            }
        }
    }

    private static void handleRemoveById(Container<? extends AbstractCommand> container, InetAddress inetAddress,
                                         int clientPort, DatagramSocket datagramSocket) throws SQLException, IOException {
        try {
            ...
            logger.error("Error in remove_by_id", e);
            sendResponse(Color.RED + "ошибка удаления: " + e.getMessage(), inetAddress, clientPort, datagramSocket);
        }
    }

    private static void handleUpdate(Container<? extends AbstractCommand> container, InetAddress inetAddress,
                                     int clientPort, DatagramSocket datagramSocket) throws SQLException, IOException {
        try {
            Integer id = container.getIntegerList().getFirst();
            ...
            sendResponse(Color.RED + "ошибка обновления: " + e.getMessage(), inetAddress, clientPort, datagramSocket);
        }
    }

    private static void handleRemoveGreater(Container<? extends AbstractCommand> container, InetAddress inetAddress,
                                            int clientPort, DatagramSocket datagramSocket) throws SQLException, IOException {
        try {
            Integer id = container.getIntegerList().getFirst();
            String owner = container.getUserArray()[0];
            ...
            logger.error("Error in remove_greater", e);
            sendResponse(Color.RED + "ошибка: " + e.getMessage(), inetAddress, clientPort, datagramSocket);
        }
    }

    private static void handleAdd(Container<? extends AbstractCommand> container, InetAddress inetAddress,
                                  int clientPort, DatagramSocket datagramSocket) throws SQLException, IOException {
        try {
            Worker worker = container.getWorker();
            if (worker == null) {
                sendResponse(Color.GREY + "пустые данные работника", inetAddress, clientPort, datagramSocket);
                return;
            }
            ...
            try {
                if (AbstractCommand.getMapCollection() == null)
                    AbstractCommand.getCollection().add(saved);
                else
                    AbstractCommand.getMapCollection().put(saved.getId(), saved);
            } finally {
                WRITE_LOCK.unlock();
            }
            sendResponse(Color.BLUE + "работник с id " + saved.getId() + " добавлен в коллекцию", inetAddress, clientPort, datagramSocket);
        } catch (Exception e) {
            logger.error("Error in add", e);
            sendResponse(Color.RED + "ошибка добавления: " + e.getMessage(), inetAddress, clientPort, datagramSocket);
        }
    }

    private static void handleAddIfMax(Container<? extends AbstractCommand> container, InetAddress inetAddress,
                                       int clientPort, DatagramSocket datagramSocket) throws SQLException, IOException {
        try {
            Worker worker = container.getWorker();
            if (worker == null) {
            ...
            if (saved == null) {
                sendResponse(Color.GREY + "работник с salary " + worker.getSalary() + " не добавлен (не больше максимума)", inetAddress, clientPort, datagramSocket);
                return;
            }
            WRITE_LOCK.lock();
            try {
            ...
            logger.error("Error in add_if_max", e);
            sendResponse(Color.RED + "ошибка: " + e.getMessage(), inetAddress, clientPort, datagramSocket);
        }
    }

    private static void handleAddIfMin(Container<? extends AbstractCommand> container, InetAddress inetAddress,
                                       int clientPort, DatagramSocket datagramSocket) throws SQLException, IOException {
        try {
            Worker worker = container.getWorker();
            if (worker == null) {
            ...
            if (saved == null) {
                sendResponse(Color.GREY + "работник с salary " + worker.getSalary() + " не добавлен (не меньше минимума)", inetAddress, clientPort, datagramSocket);
                return;
            }
            WRITE_LOCK.lock();
            try {
            ...
            logger.error("Error in add_if_min", e);
            sendResponse(Color.RED + "ошибка: " + e.getMessage(), inetAddress, clientPort, datagramSocket);
        }
    }

    private static void handleClear(Container<? extends AbstractCommand> container, InetAddress inetAddress,
                                    int clientPort, DatagramSocket datagramSocket) throws SQLException, IOException {
        try {
            String owner = container.getUserArray()[0];
            String password = container.getUserArray()[1];
            ...
            logger.error("Error in clear", e);
            sendResponse(Color.RED + "ошибка: " + e.getMessage(), inetAddress, clientPort, datagramSocket);
        }
    }

    private static void handleHelp(Container<? extends AbstractCommand> container, InetAddress inetAddress,
                                   int clientPort, DatagramSocket datagramSocket) throws IOException {
        try {
            ...
            sendResponse(Color.RED + "ошибка help: " + e.getMessage(), inetAddress, clientPort, datagramSocket);
        }
    }

    private static void handleMinByCoordinates(Container<? extends AbstractCommand> container, InetAddress inetAddress,
                                               int clientPort, DatagramSocket datagramSocket) throws SQLException, IOException {
        try {
            String sql = """
            ...
            logger.error("Error in min_by_coordinates", e);
            sendResponse(Color.RED + "ошибка: " + e.getMessage(), inetAddress, clientPort, datagramSocket);
        }
    }

    private static void handleInfo(Container<? extends AbstractCommand> container, InetAddress inetAddress,
                                   int clientPort, DatagramSocket datagramSocket) throws SQLException, IOException {
        try {
    ...
    private static void handleShow(Container<? extends AbstractCommand> container, InetAddress inetAddress,
                                   int clientPort, DatagramSocket datagramSocket) throws SQLException, IOException {
        try {
    ...
    private static Worker mapResultSetToWorker(ResultSet rs) throws SQLException {
        ...
        try {
            ...
            if (creationTimestamp != null) {
                w.setCreationDate(creationTimestamp.toLocalDateTime());
            }
        } catch (SQLException e) {
            logger.warn("Не удалось прочитать creationDate", e);
        }
        ...
        try {
            ...
            logger.error("Ошибка чтения position/status", e);
        }
