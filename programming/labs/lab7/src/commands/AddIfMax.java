...
public final class AddIfMax extends AbstractCommand {
    @Serial
    private static final long serialVersionUID = 3634819221447562719L;

    public AddIfMax(String commandName) {
        super(commandName);
    }
    @Override
    public Worker executeWithReturn() {
        try {
            ...
            for (Worker workerOfSet : getCollection()) {
                map.put(workerOfSet.getSalary(), workerOfSet);
            }
            int biggestSalary = Collections.max(map.keySet());
            if (salary <= biggestSalary) {
                System.out.println(Color.RED + "элемент не будет добавлен, так как зарплата нового работника не больше максимальной зарплаты работника в коллекции");
                return null;
            }
