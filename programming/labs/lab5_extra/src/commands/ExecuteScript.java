...
public final class ExecuteScript extends AbstractCommand {
            ...
            while ((line = bufferedReader.readLine()) != null) {
                if (line.matches("execute_script file_name [A-Za-z0-9 ]+")) {
                    ...
                    if (parts[2].equals(fileName))
                        continue;
                    innerScript(parts[2], fileName, arguments);
                } else if (line.matches("add worker [0-9A-Za-z-,]+")) {
                    parseAdd(line);
                } else {
                    try {
                        switch (line) {
                            case "info" -> info.execute(arguments);
                            case "show" -> show.execute(arguments);
                            case "add worker" -> add.execute(arguments);
                            case "update id worker" -> update_id.execute(arguments);
                            case "remove_by_id id" -> removeByID.execute(arguments);
                            case "clear" -> clear.execute(arguments);
                            case "save" -> save.execute(arguments);
                            case "exit" -> exit.execute(arguments);
                            case "add_if_max worker" -> addIfMax.execute(arguments);
                            case "add_if_min worker" -> addIfMin.execute(arguments);
                            case "remove_greater worker(s)" -> removeGreater.execute(arguments);
                            case "min_by_coordinates" -> minByCoordinates.execute(arguments);
                            case "filter_contains_name name" -> filterContainsName.execute(arguments);
                            case "filter_starts_with_name name" -> filterStartsWithName.execute(arguments);
    ...
    private void innerScript(String nameOfScript, String fileName, String arguments) throws IOException {
        ...
        try (var bufferedReader = new BufferedReader(new FileReader(file))) {
            ...
            while ((line = bufferedReader.readLine()) != null) {
                if (line.matches("execute_script file_name [A-Za-z0-9]+")) {
                    String[] parts = line.split(" ");
                    if (parts[2].equals(nameOfScript) || parts[2].equals(fileName))
                        continue;
                    innerScript(parts[2], fileName, arguments);
                } else if (line.matches("add worker [0-9A-Za-z-,]+")) {
                    parseAdd(line);
